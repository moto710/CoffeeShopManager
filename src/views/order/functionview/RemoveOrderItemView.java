...
public class RemoveOrderItemView extends OrderItemTemplate {
    ...
    protected void showBody() {
        boolean flag = false;
        do {
            System.out.println("Please enter views.order item's ID to remove:");
            Long id = Long.parseLong(scanner.nextLine());
            OrderItem temp = orderItemManagement.findIdOrderItem(id);

            if (temp == null) {
                System.out.println("The views.order item with id = " + id + " does not exist!");
                flag = true;
            } else {
                System.out.println("Are you sure to remove this views.order item? (Y/N)");
